...
export default function ResetPasswordScreen() {
  ...
  const ctaLabel = useMemo(
    () => (loading ? '' : t('resetPasswordVerifyCta')),
    [loading, t],
  );
  ...
  const handleVerify = async () => {
    if (!email || code.length !== OTP_LENGTH) {
      Alert.alert(t('commonErrorTitle'), t('resetPasswordOtpMissingFields'));
      return;
    }
  ...
  const handleResend = async () => {
    if (!email) {
      Alert.alert(t('commonErrorTitle'), t('resetPasswordEmailRequired'));
      return;
    }
    ...
    try {
      ...
      Alert.alert(
        t('resetPasswordResendTitle'),
        t('resetPasswordResendMessage'),
      );
    } catch (error: unknown) {
      Alert.alert(
        t('commonErrorTitle'),
      ...
      >
        ...
        >
          <Text className={`text-sm font-semibold ${isDark ? 'text-slate-200' : 'text-slate-700'}`}>
            {resending ? t('resetPasswordResendLoading') : t('resetPasswordResendCta')}
          </Text>
        </TouchableOpacity>
      </View>
    </AuthHeroLayout>
